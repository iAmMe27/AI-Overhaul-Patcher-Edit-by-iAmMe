...
namespace AIOverhaulPatcherEditbyiAmMe
{
    // iAm's edit - moves the settings to the same source file for easier management
    public class Settings
    {
        [SettingName("Ignore Identical to Last Override")]
        public bool IgnoreIdenticalToLastOverride { get; set; } = false;

        [SettingName("Ignore Player Record")]
        public bool IgnorePlayerRecord { get; set; } = true;

        [SettingName("Maintain Highest Protection Level")]
    ...
    public class Program
    {
        ...
        public static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
            ...
            if (AIOverhaul == null)
            {
                System.Console.WriteLine("AI Overhaul.esp not found");
                return;
            }

            // iAmMe's edit
            // create new OWL weapon key map
            var lvliMapWeap = LVLIMapWeap.CreateMap();

            // Check for Open World Loot
            var OWL = state.LoadOrder.GetModByFileName("Open World Loot.esp");

            if (OWL == null)    // not found
            {
            ...
            foreach (var npc in AIOverhaul.Npcs)
            {
                ...
                winningMaster ??= state.LoadOrder.PriorityOrder.Select(x => x.Mod).NotNull().SelectMany(x => x.Npcs).Where(x => x.FormKey == npc.FormKey).First();
                ...
                if (npc.Items != null)
                {
                    if (OWL == null)    // revert to the traditional AIO patcher method
                    {
                    ...
                    {
                        for (var i = 0; i < npc.Items.Count; i++)   // incorporate OWL changes
                        {
